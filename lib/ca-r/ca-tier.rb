...
class CA_Tier
  ...
  def add_row(row)
    ...
    @data[lookup(/County/,row)] = {
      ...
      'final_tier' => groom_integer(lookup(/Final Tier/,row,true)||lookup(/Updated Tier/,row)),
      'previous_tier' => groom_integer(lookup(/Previous Tier/,row)),
      #'starting_date' => lookup(//,row),
      'current_tier' => groom_integer(lookup(/Tier for Week/,row,true)||lookup(/Final Tier/,row,true)||lookup(/Updated Tier/,row)),
